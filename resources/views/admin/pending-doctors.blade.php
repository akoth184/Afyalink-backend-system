...
<head>
    ...
    <title>Pending Doctor Applications - AfyaLink Admin</title>
...
<body>
    <div class="admin-container">
        <div class="page-header">
            <div>
                <h1 class="page-title">Pending Doctor Applications</h1>
                <p class="page-subtitle">Review and verify doctor registration requests</p>
            </div>
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card">
                ...
                <div class="stat-value">{{ $pendingDoctors->where('created_at', '>=', now()->subWeek())->count() }}</div>
            </div>
        </div>

        <div class="card">
            <div class="card-header">
                <span class="card-title">Doctor Applications</span>
            </div>
            <div class="card-body">
                ...
                <table class="table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $doctor->facility->name ?? 'N/A' }}</td>
                            <td>{{ $doctor->created_at->format('M d, Y') }}</td>
                            <td>
                                <div class="action-btns">
                                    <form method="POST" action="{{ route('admin.doctor.approve', $doctor->id) }}">
                                        @csrf
                                        @method('POST')
                                        <button type="submit" class="btn btn-success">Approve</button>
                                    </form>
                                    <form method="POST" action="{{ route('admin.doctor.reject', $doctor->id) }}">
                                        @csrf
                                        @method('POST')
