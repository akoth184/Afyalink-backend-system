...
<head>
    ...
    <title>New Patient — AfyaLink</title>
    <link href="https://fonts.googleapis.com/css2?family=DM+Serif+Display:ital@0;1&family=DM+Sans:wght@300;400;500;600&display=swap" rel="stylesheet">
    <style>
        :root{--teal:#0d6e6e;--teal-mid:#0f8080;--teal-lt:#e6f4f4;--teal-dark:#0a5555;--green:#27ae60;--green-lt:#e8f8ef;--red:#e53e3e;--red-lt:#fff5f5;--ink:#1a1f2e;--muted:#5a6275;--border:#dde4e4;--sidebar-w:260px;--header-h:64px;--shadow-sm:0 1px 4px rgba(0,0,0,.06);--shadow-md:0 4px 16px rgba(0,0,0,.08)}
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        body{font-family:'DM Sans',sans-serif;background:#f1f5f5;color:var(--ink);min-height:100vh;display:flex}
        .sidebar{width:var(--sidebar-w);background:var(--teal-dark);min-height:100vh;position:fixed;left:0;top:0;bottom:0;display:flex;flex-direction:column;z-index:200}
        ...
        .form-card{background:white;border-radius:14px;border:1px solid var(--border);box-shadow:var(--shadow-sm);overflow:hidden;max-width:760px}
        .form-header{padding:20px 28px;border-bottom:1px solid var(--border)}
        .form-header h2{font-family:'DM Serif Display',serif;font-size:1.1rem;color:var(--ink)}
        .form-header p{font-size:.82rem;color:var(--muted);margin-top:4px}
        .form-body{padding:28px}
        .form-row{display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-bottom:20px}
        .form-group{margin-bottom:20px}
        .form-group label{display:block;font-size:.82rem;font-weight:600;color:var(--ink);margin-bottom:6px}
        .form-group input,.form-group select,.form-group textarea{width:100%;padding:10px 14px;border:1.5px solid var(--border);border-radius:9px;font-family:'DM Sans',sans-serif;font-size:.875rem;color:var(--ink);outline:none;transition:border-color .2s;background:white}
        .form-group input:focus,.form-group select:focus,.form-group textarea:focus{border-color:var(--teal);box-shadow:0 0 0 3px rgba(13,110,110,.08)}
        ...
        .section-title{font-size:.72rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--teal);margin-bottom:16px;padding-bottom:8px;border-bottom:1px solid var(--border)}
        @media(max-width:768px){.form-row{grid-template-columns:1fr}.main{margin-left:0}.sidebar{transform:translateX(-100%)}.content{padding:20px 16px}.topbar{padding:0 16px}}
    </style>
</head>
<body>
<aside class="sidebar">
    ...
    <div class="sidebar-user">
        ...
        <form method="POST" action="{{ route('logout') }}">@csrf
            <button type="submit" class="sidebar-logout"><svg viewBox="0 0 24 24"><path d="M9 21H5a2 2 0 01-2-2V5a2 2 0 012-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg></button>
        </form>
    </div>
</aside>
...
<div class="main">
    <header class="topbar">
        <div class="topbar-title">Register New Patient</div>
        <a href="{{ route('patients.index') }}" class="btn btn-sm" style="background:var(--teal-lt);color:var(--teal)">
            <svg viewBox="0 0 24 24" style="width:14px;height:14px;stroke:currentColor;fill:none;stroke-width:2.5;stroke-linecap:round;stroke-linejoin:round"><polyline points="15 18 9 12 15 6"/></svg>
            Back
        </a>
    </header>
    <main class="content">
        <div class="form-card">
            <div class="form-header">
                ...
                <p>Fill in the patient's details below</p>
            </div>
            <form method="POST" action="{{ route('patients.store') }}">
                ...
                <div class="form-body">
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="text" name="national_id" value="{{ old('national_id') }}" placeholder="12345678">
                    ...
                    <div class="section-title" style="margin-top:8px">Contact & Facility</div>
                    <div class="form-row">
                        <div class="form-group">
                            <label>Email</label>
                        ...
                        <div class="form-group">
                            <label>Phone</label>
                            <input type="text" name="phone" value="{{ old('phone') }}" placeholder="+254 7XX XXX XXX">
                            @error('phone')<div class="field-error">{{ $message }}</div>@enderror
                        </div>
                    </div>
                    <div class="form-group">
                        <label>Address</label>
                        <input type="text" name="address" value="{{ old('address') }}" placeholder="Street, Town, County">
                ...
                <div class="form-footer">
                    ...
                    <a href="{{ route('patients.index') }}" class="btn btn-sm" style="background:#f0f2f5;color:var(--muted)">Cancel</a>
                </div>
            </form>
        </div>
    </main>
